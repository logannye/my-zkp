...
impl ProverBuilder {
    /// Create a new prover builder with the given domain and AIR specification.
...
impl VerifierBuilder {
    /// Create a new verifier builder with the given domain.
...
pub mod adapters {
    ...
    impl Restreamer for VecRows {
        ...
        fn stream_rows(&self, start: RowIdx, end: RowIdx) -> Box<dyn Iterator<Item = Row> + '_> {
...
/// Proof serialization/deserialization helpers.
pub mod io {
    use super::*;
...
pub struct Tuning {
    /// Block/tile size for streaming operations.
    pub b_blk: usize,
    /// Whether to recompute the residual polynomial from scratch.
    pub strict_recompute_r: bool,
    /// Whether zeta-shift feature is enabled.
    pub zeta_shift_enabled: bool,
    /// Whether lookup arguments are enabled.
