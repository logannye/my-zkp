...
# Enable streaming mode for accurate stress testing and benchmarking
...
# 8) Stress: really big T, small b_blk
ROWS=65536
BASIS=eval
run_case "stress: really big-T smallish-b_blk (rows=65536, b_blk=128)" \
  "" "$ROWS" 128 3 "$BASIS"

# 9) Stress with zh_c=3 and dense selectors (selectors_dense.csv)
ROWS=2048
BASIS=eval
...
# 10) Stress with zh_c=7 and sparse selectors (selectors_sparse.csv)
run_case "stress: zh_c=7, sparse selectors" "" \
  "$ROWS" 128 3 "$BASIS" \
  "--selectors" "selectors_sparse.csv" "--zh-c" "7"

# 11) Mixed selector set (dense+sparse+periodic), default zh_c=1
run_case "selectors: mixed (dense|sparse|periodic), zh_c=1" "" \
  "$ROWS" 128 3 "$BASIS" \
  "--selectors" "selectors_mixed.csv"

# 12) Non-power-of-two rows (padding check) + odd/odd-ish tiles
...
# 13) Non-power-of-two rows with coeff basis (cross-product check)
run_case "padding: coeff-basis, rows=3000, b_blk=96" "" 3000 96 3 coeff

# 14) Padding × lookups (if available): non-power-of-two rows with lookups
if [[ $rc_lk -eq 0 ]]; then
...
# 15) Padding × zeta-shift (if available): non-power-of-two rows with ζ-shift
if [[ $rc_zs -eq 0 ]]; then
...
# 16) Padding × zh_c variant: change vanishing constant on padded domain
